...
18. Думай о главном (о хорошем коде, разумеется). Следуй от общего к частному.
...
_**== Получен Quest 2. Запустить скрипт src/ai_initial_module.sh в bash и следовать инструкциям. Исправить ошибки, если потребуется. Подсказка: для создания файла можно использовать перенаправление потока вывода. ==**_
...
_**== Получен Quest 4. Остановить запущенный процесс модуля src/ai_door_control.sh. Все примененные команды зафиксировать в файле quest4.sh. ==**_
...
_**== Получен Quest 5. С помощью консольного текстового редактора (nano, vi, vim, etc...) исправить конфингурацию первой двери. Выбранный редактор, а так же все примененные команды зафиксировать в файле quest5.txt. ==**_
...
\> *Перезапустить модуль конфигурации*
...
>Файлы с частями ключа будут отличаться от остальных, тебе необходимо поместить их в отдельную директорию, а мусор удалить. 
>Когда соберешь части ключа без мусора в отдельной директории - запусти в ней  **src/ai_help/unifier.sh** - я его любезно подготовил. Удачи, органический интеллект. Не подведи. Я поставил на тебя. А модуль инициализации не дал и кванта CPU.
